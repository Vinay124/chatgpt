...
const Signup = () => {
    ...
    const registerNewUser = useGoogleLogin({
        onSuccess: (response) => {
            setNewUser(response)
            ...
            try {
                if(newUser) {
                    const getResponse = await axios.get(
                        `https://www.googleapis.com/oauth2/v1/userinfo?access_token=${newUser.access_token}`,
                    {
                        headers: {
                    ...
                    if(getResponse.verified_email !== true) {
                        ...
                        localStorage.setItem('UserInformation', getResponse.data);
                        navigate('/MainUi')
                    } else {
  ...
  return (
    ...
    <div className='flex flex-col h-[800px] justify-center items-center'>
        ...
        <div className='mt-5'>
            ...
            <div className='mt-3'>
            <h6 className='font-md font-normal text-center text-sm'>Already have an account? 
                <Link to="/Signup" className='text-[#10a37f] text-md mx-2'>Log in </Link></h6>
            </div>

            <div className='flex justify-center bg-[#c2c8d0] relative h-[1px] rounded-xl  mt-[30px]'>
                <span className='px-2 py-1 text-center absolute font-normal text-gray-500 bottom-[-18px] bg-white  mb-1'>OR</span>
            </div>

            <div className='mt-5 py-3'>
                <button className='py-2.5 px-5 mb-3 text-sm  w-full font-medium text-[#2d333a] focus:outline-none rounded-md border border-[#c2c8d0]  focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:bg-[#d9ebf3]  dark:text-[#2d333a] dark:border-[#c2c8d0] dark:hover:text-black dark:hover:bg-[#f2f2f2] flex items-center' onClick={registerNewUser}><FcGoogle size={30} className='mr-5'/>  Continue with Google</button>
